...
void display_text(int count) //커서이동 
{
	gotoxy(1,2);
...
void display_question(void)
{
	...
	while(!feof(fp1))
	{
		fgets(chr1, 80, fp1);//질문에 저장된 텍스트 파일을 읽어 들임 
		if (chr1[0]==10) //다섯글자 이내 
		{
			display_text(count);
			gotoxy(1, line);
			scanf("%s", sol[count]); //질문을 표시하고 응답자의 선택을 입력받음 
			system("cls");
			line=5;
			count++;
		}
...
void display_result(void)
{
	...
	fp2=fopen(file2, "r");
	system("cls");
	while(!feof(fp2)) //해설이 저장된 텍스트 파일을 읽어 들임 
	{
		fgets(chr1, 80, fp2);
		if (chr1[0]==10)
		{
			gotoxy(1, line+1);
			printf("당신의 답은 : %s\n", sol[count]); //각 질문에 대한 해설을 표시하고 응답자의 선택을 표시 
