...
int main(void)
{
	//릴에 표시할 기호, 릴의 숫자 금액에 대한 초기화 
	char reel[6][3]={"★", "♠", "◆", "♥", "♣", "○"};
	int  i, reel_num[3][3];
	int money=10000;
	srand(time(NULL));
	printf("  슬롯머신 게임\n\n");
	display_rule(); //당첨 사례와 사례금액을 출력하는 함수 
	gotoxy(3, 5); //커서의 위치 제어 
	printf("릴1 릴2 릴3");
	gotoxy(1,6); //커서의 위치 제어
	draw_check02(3, 3); //사각형 격자모양 출력 
	for(i=0;i<3;i++)
		reel_num[0][i]=rand()%5;
	reel_series(reel_num);//각 릴에 대한 회전 순서를 결정 
...
//int r[][3] -> 화면에 표시할 세개의 릴에 대한 기호의 순서를 정하기 위해 사용 
}


//갱신된 내용을 표시하기 전에 화면에 지시문들을 지운다. 
void clear_text(void)
{
	int i, j;
	for(i=17;i<20;i++)
